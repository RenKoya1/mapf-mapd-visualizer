...
const PixiApp = forwardRef(
  (
    ...
    useImperativeHandle(ref, () => ({
      skipBackward: () => {
        timestepRef.current = Math.max(
          0,
          timestepRef.current - stepSizeRef.current
        );
      },
      skipForward: () => {
        if (solution) {
          timestepRef.current = Math.min(
            timestepRef.current + stepSizeRef.current,
            solution.length - 1
          );
        }
    ...
    const moveAndRotateSprites = useCallback(
      (agents: PIXI.Container[], currentTime: number) => {
        if (!solution) return;
        ...
        const nextState =
          solution[Math.min(currentTimestep + 1, solution.length - 1)];
        ...
        agents.forEach((agent, index) => {
          ...
          if (
            ...
            circleContainer.children.length < 2
          )
            return;
    ...
    const updatePaths = useCallback(
      (agents: PIXI.Container[], currentTime: number) => {
        ...
        agents.forEach((_agent, index) => {
          ...
          const full_segments = agentPathsRef.current.full.children[
            index
          ] as PIXI.Container;
          const partial_segments = agentPathsRef.current.partial.children[
            index
          ] as PIXI.Container;
          ...
          if (
            ...
            currentTimestep < solution.length - 1
          ) {
            ...
            segment.lineTo(
              scalePosition(interpolatedPosition.x),
              scalePosition(interpolatedPosition.y)
            );
            segment.stroke(agentLineStyle);
          }
        });
    ...
    const updateGoalVectors = useCallback(
      (agents: PIXI.Container[]) => {
        ...
        agents.forEach((agent, index) => {
          const goalVector = goalVectorsRef.current.children[
            index
          ] as PIXI.Graphics;
    ...
    const animateSolution = useCallback(() => {
      ...
      const orientationAware: boolean =
        solution[0][0].orientation !== Orientation.NONE;
      ...
      solution[0].forEach((_pose, agentId) => {
        ...
        circle.circle(0, 0, GRID_UNIT_TO_PX / 3).fill(agentColor);
        if (orientationAware) {
      ...
      const animate = () => {
        if (timestepTextRef.current) {
          timestepTextRef.current.text = `${timestepRef.current.toFixed(
            1
          )} / ${(solution.length - 1).toFixed(1)}`;
        }
        ...
        moveAndRotateSprites(
          agents.children as PIXI.Container[],
          timestepRef.current
        );
    ...
    const drawGrid = useCallback(() => {
      ...
      for (let x: number = 0; x < graph.width; x++) {
        for (let y: number = 0; y < graph.height; y++) {
          ...
          const idText = cellContainer.addChild(
            new PIXI.Text({
    ...
    useEffect(() => {
      ...
      if (goalVectorsRef.current)
        goalVectorsRef.current.visible = showGoalVectors;
